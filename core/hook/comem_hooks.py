    ...
    def evaluate(self, current_step):
        ...
        for video_name in self.trainer.test_dataset_keys_w:
            ...
            for test_data, anno, meta in data_loader:
                ...
                gtFlow_vis, gtFlow = flow_batch_estimate(
                    self.trainer.flownet,
                    input_test,
                    self.trainer.normalize.param['val'],
                    output_format=self.trainer.config.DATASET.optical_format,
                    optical_size=self.trainer.config.DATASET.optical_size)
                ...
                diff_appe, diff_flow = simple_diff(future_test, app_output,
                                                   gtFlow, motion_output)
                patch_score_appe, patch_score_flow, _, _ = find_max_patch(
                    diff_appe, diff_flow)
        ...
        for sn, video_name in enumerate(self.trainer.test_dataset_keys):
            ...
            for frame_sn, (data, anno, meta) in enumerate(data_loader):
                ...
                score, _, _ = amc_score(future_test, app_output, gtFlow,
                                        motion_output, wf, wi)
                ...
                if sn == random_video_sn and (frame_sn in vis_range):
                    temp = vis_optical_flow(
                        ...
                        output_format=self.trainer.config.DATASET.
                        optical_format,
                        output_size=(motion_output.shape[-2],
                                     motion_output.shape[-1]),
                        normalize=self.trainer.normalize.param['val'])
                    vis_objects = OrderedDict({
                        'comem_eval_frame':
                        future_test.detach(),
                        'comem_eval_frame_hat':
                        app_output.detach(),
                        'comem_eval_flow':
                        gtFlow_vis.detach(),
                        'comem_eval_flow_hat':
        ...
        self.trainer.pkl_path = save_results(
            self.trainer.config,
            self.trainer.logger,
            verbose=self.trainer.verbose,
            config_name=self.trainer.config_name,
            current_step=current_step,
            time_stamp=self.trainer.kwargs["time_stamp"],
            score=score_records,
            psnr=psnr_records)
        results = self.trainer.evaluate_function(
            self.trainer.pkl_path, self.trainer.logger, self.trainer.config,
            self.trainer.config.DATASET.score_type)
        self.trainer.logger.info(results)
        tb_writer.add_text('CoMemAE: AUC of ROC curve',
