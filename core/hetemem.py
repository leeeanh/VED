...
class Trainer(DefaultTrainer):
    ...
    def custom_setup(self):
        if self.kwargs['parallel']:
            self.HeteMem = self.data_parallel(self.model['HeteMem'])
            self.flownet = self.data_parallel(self.model['FlowNet'])
        else:
            self.HeteMem = self.model['HeteMem'].cuda()
            self.flownet = self.model['FlowNet'].cuda()

        # get the optimizer
        self.optim_HeteMem = self.optimizer['optimizer_HeteMem']
        ...
        # the lr scheduler
        self.lr_HeteMem = self.lr_scheduler_dict['optimizer_HeteMem_scheduler']

        # basic meter
        self.loss_meter_HeteMem = AverageMeter(name='loss_HeteMem')
    ...
    def train(self, current_step):
        ...
        # initialize
        start = time.time()
        self.set_requires_grad(self.HeteMem, True)
        self.HeteMem.train()
        self.flownet.eval()
        ...
        # run our model
        motion_output, motion_fea, motion_updated_fea, app_output, \
        app_fea, app_updated_fea, app_separateness_loss, app_compactness_loss, \
        motion_separateness_loss, motion_compactness_loss = self.HeteMem(inputs, train=True)
        ...
        # optimizer
        self.optim_HeteMem.zero_grad()
        loss_all.backward()
        self.optim_HeteMem.step()

        # Update meter loss
        self.loss_meter_HeteMem.update(loss_all.detach())
        ...
        if (current_step % self.steps.param['log'] == 0):
            msg = make_info_message(current_step, self.steps.param['max'],
                                    ...
                                    self.data_time, [self.loss_meter_HeteMem,
        ...
        writer.add_scalar('Train_loss_HeteMem', self.loss_meter_HeteMem.val,
                          global_steps)
        ...
        if (current_step % self.steps.param['vis'] == 0):
            ...
            vis_objects = OrderedDict({
                'train_output_app_HeteMem':
                app_output.detach(),
                'train_output_motion_HeteMem':
                temp,
                'train_input':
                input_data.detach()
            })
        ...
        # save model
        self.saved_model = {'HeteMem': self.HeteMem}
        self.saved_optimizer = {'optim_HeteMem': self.optim_HeteMem}
        self.saved_loss = {'loss_HeteMem': self.loss_meter_HeteMem.val}
        self.kwargs['writer_dict']['global_steps_{}'.format(self.kwargs['model_type'])] = global_steps

    def mini_eval(self, current_step):
        temp_meter_frame = AverageMeter()
        temp_meter_flow = AverageMeter()
        self.set_requires_grad(self.HeteMem, False)
        self.HeteMem.eval()
        for data, _, _ in self.val_dataloader:
            ...
            app_compactness_loss = self.HeteMem(inputs, train=False)
...
class Inference(DefaultInference):
    ...
    def custom_setup(self):
        if self.kwargs['parallel']:
            self.HeteMem = self.data_parallel(self.model['HeteMem']).load_state_dict(self.save_model['HeteMem'])
        else:
            self.HeteMem = self.model['HeteMem'].cuda()
            self.flownet = self.model['FlowNet'].cuda()
            self.HeteMem.load_state_dict(self.save_model['HeteMem'])
