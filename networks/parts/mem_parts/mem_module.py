...
class Memory(nn.Module):
    def __init__(self, memory_size, feature_dim, key_dim, temp_update,
                 temp_gather):
        ...
        self.keys = Parameter(
            F.normalize(torch.rand((memory_size, feature_dim),
                                   dtype=torch.float),
    ...
    def forward(self, query, train=True):
        ...
        if train:
            ...
            separateness_loss, compactness_loss = self.gather_loss(
                query, self.keys, train)
            # read
            updated_query, softmax_score_query, softmax_score_memory = self.read(
                query, self.keys)
            #update
            updated_memory = self.update(query, self.keys, train)
    ...
    def update(self, query, keys, train):
        ...
        if train:
            ...
            query_update = self.get_update_query(keys, gathering_indices,
                                                 updating_indices,
                                                 softmax_score_query,
                                                 query_reshape, train)
    ...
    def pointwise_gather_loss(self, query_reshape, keys, gathering_indices,
                              train):
        n, dims = query_reshape.size()  # (b X h X w) X d
        loss_mse = torch.nn.MSELoss(reduction='none')

        pointwise_loss = loss_mse(query_reshape,
    ...
    def gather_loss(self, query, keys, train):
        ...
        if train:
            ...
            return gathering_loss, query_reshape, keys[
                gathering_indices].squeeze(1).detach(), gathering_indices[:, 0]
    ...
    def read(self, query, updated_memory):
        ...
        concat_memory = torch.matmul(softmax_score_memory.detach(),
                                     updated_memory)  # (b X h X w) X d
        updated_query = torch.cat((query_reshape, concat_memory),
                                  dim=1)  # (b X h X w) X 2d
