...
class MemoryRamTwoStreamModule(nn.Module):
    ...
    def __init__(self, input_size, hidden_size=512, memory_bank_size=100):
        ...
        self.keys = Parameter(
            F.normalize(torch.rand((memory_bank_size, input_size),
                                   dtype=torch.float),
    ...
    def forward(self, hidden_out_a, hidden_out_m, nImg, train):
        memory_ram = torch.FloatTensor(self.memory_bank_size, self.hidden_size).cuda()
        memory_ram.fill_(0)
        ...
        hiddens_h_t_a = torch.FloatTensor(nImg, self.hidden_size).cuda()
        hiddens_h_t_m = torch.FloatTensor(nImg, self.hidden_size).cuda()
        hiddens = torch.FloatTensor(nImg, self.hidden_size).cuda()

        query_a = F.normalize(hidden_out_a, dim=1)
        query_a = query_a.permute(0, 2, 3, 1)
        query_m = F.normalize(hidden_out_m, dim=1)
        query_m = query_m.permute(0, 2, 3, 1)
        ...
        for t in range(nImg):
            ...
            hiddens_h_t_a[t, :] = h_t_a
            hiddens_h_t_m[t, :] = h_t_m
            hiddens[t, :] = h_t

        if train:
            #losses
            app_separateness_loss, app_ompactness_loss = mem_module.gather_loss(
                query_a, self.keys, train)
            motion_separateness_loss, motion_app_ompactness_loss = mem_module.gather_loss(
                query_m, self.keys, train)
            return hiddens_h_t_a, hiddens_h_t_m, app_separateness_loss, app_ompactness_loss, motion_separateness_loss, motion_app_ompactness_loss
        else:
            #losses
            app_compactness_loss, _, _, _ = mem_module.gather_loss(
                query_a, self.keys, train)
            motion_compactness_loss, _, _, _ = mem_module.gather_loss(
                query_m, self.keys, train)
            return hiddens_h_t_a, hiddens_h_t_m, app_compactness_loss,  motion_compactness_loss
